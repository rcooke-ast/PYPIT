...
#                          ' None means no user-level slits defined.'
    ...
    def validate(self):
        ...
        for k in ['rm_slits', 'add_slits']:
            if self[k] is None or (self[k] is not None and not any('(' in s for s in self[k])):
                continue
            # Find the list indices that include the opening and closing parentheses
            indx = np.where(['(' in s or ')' in s for s in self[k]])[0]
            if indx[0] != 0 or len(indx) % 2 != 0:
                # There must always be an open parenthesis in the first list
                # element, and there must be an even number of elements with
                # either ( or ).  NOTE: This means nested parentheses will not
                # work!
                msgs.error(f'Could not interpret provided value(s) for {k}.  Check for any '
                           'unpaired parentheses.')
            self[k] = [','.join(l) for l in np.split(self[k], (indx+1)[1:-1:2])]
